{ dotdir, ... }:
{
  programs.bash = {
    ...
    shellAliases = {
      "arise" = ''notify-send "Shadow extraction in progress ..."; (sudo nixos-rebuild switch --flake ~/${dotdir} --show-trace && notify-send "Shadow extraction succeeded.") || notify-send "Shadow extraction failed."'';
